...
| 术语 | 大白话解释 | 生活类比 |
|------|-----------|----------|
| 🔗 **生成模型（Generative model）** | 学习数据本身分布 $P(X)$，能采样造新样本 | 学画家画风后自己画新画 |
| 🔗 **判别模型（Discriminative model）** | 学习条件分布 $P(Y\|X)$，做分类/标注 | 鉴定师：只判断真假，不学会造假 |
| 🔗 **潜在空间 $Z$（Latent space）** | 高维数据 $X$ 的低维压缩表征 | 乐谱：几百个音符概括整首曲子 |
| 🔗 **紧致性（Compactness）** | $Z$ 低维、信息密集，受先验约束不散落 | 图书馆按分类码整齐排架，不是每本书独占一格 |
