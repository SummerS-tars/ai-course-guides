...
**本 Part 要解决的问题**：为什么本课件不从显式规则出发，而从大量简单单元和连接权重出发解释智能？复习时先抓住「知识藏在权重里」这条主线，再进入 BP 推导。
...
> **直观理解**：联结主义（Connectionism）把智能看成许多简单节点并行作用的结果；它不是「突然火起来」，而是经历了感知机寒冬后，靠可训练深层网络重新进入主流。
...
**先分清场景**：符号主义适合规则清楚、能写成逻辑的任务；联结主义适合图像、语音、模式识别这类规则难手写但样本较多的任务。
...
**本 Part 要解决的问题**：多层网络要能学习，必须让「输出错了多少」能沿公式一路传回权重；因此激活函数要可导，损失函数要可求导，优化算法要知道朝哪边改权重。
...
**使用场景**：MSE（Mean Squared Error，均方误差）常用于回归或课件中的推导例，因为它把「预测离目标有多远」变成一个可求导的标量。
...
梯度 $\nabla L(\theta_t)$ 指向损失上升最快方向，所以更新时取负号，表示沿「下山」方向改参数：
...
**本 Part 要解决的问题**：输出层错误容易算，隐层没有直接目标值，BP（Backpropagation，反向传播）用链式法则把输出误差逐层分摊给前面权重，解决多层网络的权责分配问题。
...
**符号约定**：本指南采用课件常见约定 $\delta = -\partial L/\partial z$，因此权重修正写成「加上」$\eta \delta h$；若代码里把梯度定义为 $\partial L/\partial z$，更新式会写成减去梯度，二者不能混用。
...
> **PJ1 实现要点**：在本节 $\delta=-\partial L/\partial z$ 约定下，输出层 $\delta$ 统一为 **真实 − 预测**；若代码按 PyTorch 梯度习惯记作 $g=\partial L/\partial z$，则 $g= y-t$。隐层传播仍须乘 $f'(z)$。
...
**范式**：**无监督学习 Unsupervised Learning**，即没有教师标签，模型只能从输入之间的相似性里找结构。
...
**机制拆解**：某节点多次赢得相似样本后，它的权向量会被反复拉向这一类输入；久而久之，不同节点分别代表不同输入簇，所以 WTA 可作为聚类直觉理解。
...
- 在本指南 $\delta=-\partial L/\partial z$ 约定下，输出层 $\delta$ 可统一为 $t-y$（配对正确前提下）；代码里的梯度常写成 $y-t$。
