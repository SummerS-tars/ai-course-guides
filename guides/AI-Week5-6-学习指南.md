...
| 术语 | 大白话解释 | 生活类比 |
|------|-----------|----------|
| 🔗 **序列建模** | 输入/输出随时间排列，前后有关联 | 看电影：不能只看某一帧 |
| 🔗 **隐状态** | 看不见但决定观测的内部状态 | 地下室警卫猜天气，只能看主管带不带伞 |
| 🔗 **观测** | 能直接看到的数据 | 雨伞、汉字、声学信号 |
| 🔗 **马尔可夫性** | 未来只依赖当前，不依赖更早历史 | 明天天气主要看今天，不太看前天 |
| 🔗 **HMM 五元组** | 状态集、观测集、初始分布、转移矩阵、发射矩阵 | 一台「先藏状态、再吐符号」的随机机器 |
| 🔗 **评估问题** | 给定模型，算观测序列出现概率 $P(O\|\lambda)$ | 「这段话像不像这个模型生成的？」 |
| 🔗 **解码问题** | 给定观测，找最可能的隐状态序列 | 「每个字最可能是什么词性/分词标签？」 |
| 🔗 **学习问题** | 只有观测，估计模型参数 | 「没见过隐状态，靠数据反推机器参数」 |
| 🔗 **前向概率 $\alpha_t(i)$** | 到时刻 $t$、状态 $i$、且已看到 $O_1\ldots O_t$ 的累积概率 | 走到棋盘第 $t$ 列、站在格子 $i$ 的「到达分」 |
| 🔗 **维特比 $\delta_t(i)$** | 到时刻 $t$、状态 $i$ 的**最优路径**概率 | 只保留冠军路线，不要所有路线平均分 |
| 🔗 **Baum-Welch** | HMM 的 EM 学习算法 | 看不见隐状态，用「软计数」反复调参 |
| 🔗 **标签偏置** | 局部归一化让模型偏爱「后继分支少」的状态 | 岔路只有一条时，被迫把概率全押上去 |
| 🔗 **CRF** | 直接建模 $P(Y\|X)$ 的判别式序列模型 | 不研究数据怎么生成，只研究怎么标标签 |
| 🔗 **全局归一化 $Z(X)$** | 对所有可能标签路径统一归一化 | 全国统一划线，不是每家门自己算百分制 |
| 🔗 **特征函数 $\phi_k$** | 衡量「观测+标签组合」是否匹配的打分项 | 人工写的「如果…则加一分」模板 |
| 🔗 **词嵌入** | 把词映射到低维稠密向量 | 给每个词一张「属性名片」，相近词靠近 |
| 🔗 **窗口模型** | 固定窗口拼接词向量 → FC → Softmax | 只看前后几个字，给当前字贴标签 |
| 🔗 **BIES** | 分词标签：B 词首 / I 词中 / E 词尾 / S 单字词 | 给每个汉字标它在词里的位置 |
