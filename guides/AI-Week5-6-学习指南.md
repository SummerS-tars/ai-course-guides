...
| 🔗 **HMM 五元组（Hidden Markov Model）** | 状态集、观测集、初始分布、转移矩阵、发射矩阵 | 一台「先藏状态、再吐符号」的随机机器 |
...
| 🔗 **Baum-Welch 算法** | HMM 的 EM 学习算法 | 看不见隐状态，用「软计数」反复调参 |
| 🔗 **标签偏置（Label bias）** | 局部归一化让模型偏爱「后继分支少」的状态 | 岔路只有一条时，被迫把概率全押上去 |
| 🔗 **条件随机场（CRF, Conditional Random Field）** | 直接建模 $P(Y\|X)$ 的判别式序列模型 | 不研究数据怎么生成，只研究怎么标标签 |
...
| 🔗 **BIES 标签（Begin/Inside/End/Single）** | 分词标签：B 词首 / I 词中 / E 词尾 / S 单字词 | 给每个汉字标它在词里的位置 |
...
### 2.2 Week 6：条件随机场（CRF）、词嵌入（Word embedding）与早期序列 DL
