...
- HMM 五元组 $\lambda=(S,V,\pi,A,B)$ 与生成过程
- 三基本问题：评估 / 解码 / 学习
...
**回溯**：$\hat{X}*t = \psi*{\hat{X}_{t+1}}(t+1)$，从 $T$ 倒推到 1
...
> **与 PJ2 的关系**（来源：`w56-project`、`w12-pj2`）  
...
> **结论**：PJ2 就是 Week 5–6 的**动手版**——同一分词任务上串起 HMM → CRF → DL，体会「统计 → 判别 → 神经网络表征」。

| PJ2 要求 | 对应 Week 5–6 | 指南章节 |
|---------|--------------|---------|
| 手写 **HMM**（EM 训练 + Viterbi 解码） | Week 5 | §2.1 C–H |
| 手写 **CRF**（特征模板 + 全局归一化） | Week 6 | §2.2 C–E |
| **DL 对比模型**（Embedding + 窗口 / LSTM；后期可 Transformer） | Week 6 | §2.2 F–G；Transformer 见 Week 12 |
| **中文分词 BIES** | 任务设定（三模型共用） | 术语表；§2.2 G 例句 |
| 面试查原理 | EM、Label Bias、BP 到 Embedding | §2.1 H；§2.2 B、F |

**三条线的分工**：

```
同一批分词数据
  ├─ HMM：隐状态 = 标签，Baum-Welch 学 A,B,π，Viterbi 出标签
  ├─ CRF：人工特征 + Z(X)，全局解码
  └─ DL：Embedding 替 One-hot/手工特征，BP 端到端训窗口或 RNN
```

**严禁**：`sklearn` CRF 接口、现成分词 / `transformers` 封装；可用 `numpy`、自写 PyTorch 层。

（来源：`w56-project`、`w12-pj2`、Week 5/6 记录）
