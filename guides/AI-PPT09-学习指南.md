...
**本 Part 要解决的问题**：深度学习为什么不是「多堆几层」这么简单？它的核心价值是把原始数据逐层变成更适合任务的表示，从而减少人工特征工程。
...
- **鲁棒性 Robustness**：输入有小噪声或轻微位移时，好的表示仍保留任务相关信息，使下游分类器压力减小、性能跃升。
...
**结构**：编码器 $X \to Z$ + 解码器 $Z \to \hat{X}$，最小化重构误差。白话说，AE 先把输入压缩成瓶颈表示 $Z$，再尝试还原输入；如果还原得好，说明 $Z$ 抓住了主要信息。
...
- 层内无连接、层间全连接的双向随机网络；「受限」指去掉层内连接，使训练近似更可处理。
...
**本 Part 要解决的问题**：图像像素数量巨大，若全连接会参数爆炸；CNN（Convolutional Neural Network，卷积神经网络）用局部连接和权重共享，把「在不同位置找同一种局部模式」写进结构里。
...
**先看变量含义再套公式**：$I$ 是输入边长，$K$ 是卷积核边长，$P$ 是两侧各补多少圈 0，$S$ 是每次滑动几格；分子表示「补边后的有效可滑动长度」。
...
**池化 Pooling**：无参数下采样，用小窗口把局部区域汇总成一个值；它牺牲精确位置，换来更小尺寸和局部平移不敏感。
...
**本 Part 要解决的问题**：文本、语音这类数据不是孤立样本，而是前后相关的序列；传统概率模型先解决「隐藏标签如何随时间变化」，再过渡到词向量和神经网络表示。
...
**HMM（Hidden Markov Model，隐马尔可夫模型）**：我们看得见的是观察序列 $O$（词、字、语音帧），看不见的是隐藏状态 $S$（词性、分词标签等）；模型假设当前状态主要由前一状态决定。
...
| $B$ | 发射概率，例如 $b_j(k)=P(K_k \mid S_j)$，表示状态 $S_j$ 下生成观察符号 $K_k$ 的概率 |
...
**机制拆解**：暴力枚举要尝试 $N^T$ 条隐藏状态路径；前向算法把「到达同一状态的所有历史路径概率」合并成一个 $\alpha_i(t)$，下一步只沿状态转移矩阵继续递推。

（来源：课件09 Part III、Week 5）

### 块 III.3 CRF vs HMM

**CRF（Conditional Random Field，条件随机场）**：直接建模给定输入序列后的标签概率 $P(Y \mid X)$，白话说是「看到整句后给每个位置打标签」，因此能放入更丰富的上下文特征。
...
**考试定位**：理解 CBOW / Skip-gram 的输入输出方向和「上下文决定语义」即可；训练加速技巧如负采样一般不是本课件手算重点。
...
**本 Part 要解决的问题**：CNN 擅长局部空间模式，RNN（Recurrent Neural Network，循环神经网络）试图把历史信息装进隐藏状态，用同一组权重逐步处理序列。
...
- 因 $W$ 在时间步上**重复相乘**，梯度含 $W^n$；时间越长，相乘次数越多。
...
**机制拆解**：遗忘门先决定旧记忆保留多少，输入门决定新候选写入多少，输出门决定当前记忆暴露给 $h_t$ 多少；门值接近 0 表示关，接近 1 表示放行。
...
**本 Part 要解决的问题**：深层网络能表达复杂函数，但训练不稳、易过拟合；优化器负责走得更快更稳，初始化/归一化/正则化负责让训练过程不崩。
...
**易混提醒**：BN 的统计对象是 mini-batch 维度，LN 的统计对象是单样本内部特征维度；所以变长序列、小 batch 或自回归生成场景通常更偏向 LN。
