...
```
Week 2 遗产：MLP 结构 + 梯度下降 + Mini-batch
    ↓
Week 3 主线：BP 全景 → 符号地基 → 输出层推导 → 隐层传递 → 训练循环 → 手算验证
    ↓
Week 4 主线：PJ1 地图 → 梯度消失/ReLU → CNN 为何存在 → 卷积参数 → 池化 → 特征层级
    ↓
收尾：Week 1–2 衔接 → PJ1 映射 → 易错点 → 复习优先级
```
