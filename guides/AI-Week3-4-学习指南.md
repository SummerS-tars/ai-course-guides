...
| 🔗 **反向传播（Backpropagation, BP）** | 从输出端往回，用链式法则把误差分摊到每一层权重 | 从最后一道题倒推，找出每一步错在哪 |
...
| 🔗 **批量归一化（Batch Normalization, Batch Norm）** | 按 batch 归一化特征，再学缩放/平移 | 每层楼先做「标准化体检」再激活 |
...
| **Week 4** | 图像数据怎么高效处理？ | 解释卷积与 CNN 动机、设卷积参数、理解深层网络训练 |
...
flowchart TB
    ...
    W4 --- W4a[CNN导览 → ReLU → 卷积直觉 → 参数 → 池化 → 特征层级]
...
后面 J–N 节会把这个轮廓填满：J 节解决「能堆多深」，K 节讲清卷积与 CNN 动机，L/M 节讲参数与池化，N 节讲层越深特征越抽象。**Week 7（§2.3）** 在此基础上补数学定义、互相关、多通道与 LeNet 手算。
...
>     ↓
> P. 离散卷积/LTI →  课本里的 y[n]=Σx[m]h[n-m] 从哪来
>     ↓
...
Week 4 K 节的「滑动点积」，在信号处理里有严格名字：**离散卷积（discrete convolution）**。
...
| 符号 | 含义 |
|------|------|
| $x[m]$ | 输入信号（一维序列；图像是二维，道理相同） |
| $h[m]$ | **卷积核 / 冲激响应**（滤波器） |
| $y[n]$ | 输出：在位置 $n$ 处，输入与「翻转后的核」重叠区域的加权和 |
| $n - m$ | 关键：**核要翻转**（$h[m] \to h[n-m]$）才是数学上的卷积 |

**LTI 系统在说什么？**

**线性时不变（LTI）** 系统有两条性质：

- **线性**：输入的加权和 → 输出的加权和（叠加原理）
- **时不变**：规则不随时间/位置变——输入延迟多少，输出也延迟多少，波形不变

生活类比：音叉——今天敲和明天敲，衰减规律一样（时不变）；敲两下等于分别敲的声音叠加（线性）。

卷积是 LTI 的标配运算：输出 = 输入与系统**冲激响应** $h$ 的卷积。CNN 每层学的就是一组可训练的「局部冲激响应」。

**为何核要翻转？**

公式里出现 $h[n-m]$ 而非 $h[m]$，数学上核被**翻转**了。主要为了：

1. 满足**交换律** $x*h = h*x$、**结合律**——信号处理推导里很重要
2. 对应因果直觉：「过去的输入」对「现在 $n$」的贡献，经历时长为 $n-m$

**二维情形**（数学卷积）

$$Y[i,j] = \sum_{u,v} X[i-u,\, j-v] \cdot H[u,v]$$

（来源：Week 7 记录、课件 08；raw：`w7-lti-discrete-conv`）

---

#### Q. CNN 里的互相关：和数学卷积差在哪

**行业惯例**：深度学习框架（PyTorch、TensorFlow）里的 `conv2d`，以及 Week 4 K 节教的「核不翻转、直接对齐相乘求和」，严格说是**互相关（cross-correlation）**：
