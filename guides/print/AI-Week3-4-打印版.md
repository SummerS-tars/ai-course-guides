...
打印版恢复图：这张图保留 Week 3、Week 4、Week 7 的学习主线，适合先建立全局顺序再看细节。
...
flowchart TB
    ...
    W3 --- W3a["全景 → z/h → 输出层δ → 隐层δ → 训练循环"]
    W4 --- W4a["CNN导览 → 卷积直觉 → 参数 → 池化 → 特征层级"]
    W7 --- W7a["离散卷积 → 互相关 → 多通道 → BN → LeNet手算"]
```
...
打印版恢复图：前向缓存与反向问责是 BP 的核心，保留这张图便于对照公式。
...
$$\delta_k = \frac{\partial L}{\partial z_k} = \underbrace{\frac{\partial L}{\partial y_k}}_{y_k - t_k} \cdot \underbrace{\frac{\partial y_k}{\partial z_k}}_{1} = y_k - t_k$$
...
> 损失 L → 输出层 δ_k → 隐层 h_j 的责任 → 隐层 δ_j → ∂L/∂w_ji
...
> 打印提示：完整版此处为流程图；打印版保留为文字复习线索。
> 输入图像 / 像素网格 → 卷积层 / 滑动模板提取局部图案 → 特征图 / 哪里像哪种图案 → 池化层 / 缩小 + 变稳 → 更抽象、更稳定 / 的特征
...
中心权重大、周围为负；平坦区抵消为 0，边缘处正负无法抵消 → 亮响应。
...
**课程数值例**：RGB 输入 $C_{in}=3$，$K=5$，要 $C_{out}=64$ 种特征：
