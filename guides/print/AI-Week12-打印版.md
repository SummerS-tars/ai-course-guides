...
打印版恢复图：这张图保留 Week 12 在序列建模、PJ2 和符号主义之间的承上启下位置。
...
打印版恢复图：Q/K/V 检索链路是 Transformer 的核心直觉，适合保留在公式前。
...
> 已生成 token 嵌入 → ① 因果 MHAttn / Mask 未来位置 → ② 交叉 MHAttn / Q←Dec, K/V←Enc → Encoder 输出 → ③ FFN → 输出 + 残差/LN
...
> 打印提示：完整版此处为流程图；打印版保留为文字复习线索。
> 流匹配 Flow Matching / ODE 学速度场 → 扩散模型 SDE / 多步去噪 → VAE / 一步生成
