...
| **判别模型（Discriminative model）** | 学习条件分布 $P(Y \mid X)$，做分类/标注 | 鉴定师：只判断真假，不学会造假 |
...
| **编码器 $q_\phi(z \mid x)$（Encoder）** | 把观测 $x$ 映射为 $z$ 的分布参数 | 压缩软件：把照片压成 zip |
| **解码器 $p_\theta(x \mid z)$（Decoder）** | 从 $z$ 重建 $x$ | 解压软件：从 zip 还原照片 |
...
打印版恢复图：这张全景图说明 Week 8 如何从判别模型转向生成模型，并向 Week 10 的优化工具过渡。
...
与 Week 6 CRF 的对比：CRF 直接建模 $P(Y \mid X)$，绕开 $P(X)$ 的困难；生成模型把视角放大到数据本身。
...
打印版恢复图：潜在空间的两股约束是理解 VAE 与 ELBO 的关键。
...
> 观测 x → 编码器 μ,σ → ε~N(0,I) → z=μ+σε → 0,I → 解码器 → 重构项 → KL 项
...
| $q_\phi(z \mid x)$ | 编码器：近似后验 |
| $p_\theta(x \mid z)$ | 解码器：生成分布 |
| $\mathcal{L}(x;\phi,\theta)$ | ELBO，$\log p(x)$ 的下界 |

**推导摘要**：

$$\log p(x) = \underbrace{\mathbb{E}_{z \sim q_\phi} \left[\log \frac{p_\theta(x,z)}{q_\phi(z \mid x)}\right]}_{\text{ELBO}} + \underbrace{D_{KL}(q_\phi(z \mid x) \| p_\theta(z \mid x))}_{\ge 0}$$

因 $D_{KL} \ge 0$，最大化 ELBO 等价于间接最大化 $\log p(x)$。代入 $p_\theta(x,z)=p_\theta(x \mid z)p(z)$：

$$\mathcal{L}(x;\phi,\theta) = \underbrace{\mathbb{E}_{z \sim q_\phi}[\log p_\theta(x \mid z)]}_{\text{重构项}} - \underbrace{D_{KL}(q_\phi(z \mid x) \| p(z))}_{\text{KL 正则项}}$$
...
> **承接 D 节**：ELBO 需要 $z \sim q_\phi(z \mid x)$，但「采样」阻断梯度。重参数化把随机性转移到独立噪声 $\varepsilon$ 上。
...
| 关系 | $\log p(x) = \text{ELBO} + D_{KL}(q \| p(z \mid x))$ | 真实后验 $p(z \mid x)$ 不可算，用 $q$ 近似 |
