...
打印版恢复图：这张图保留 Week 10 的工程主线，从生成模型训练难点走向优化器与稳定化工具箱。
...
打印版恢复图：海森矩阵把优化难点和对应处理手段连在一起，是本周最值得保留的流程图。

```mermaid
flowchart TB
    H["海森矩阵 H"]
    H --> SP["鞍点<br/>特征值正负混合<br/>梯度≈0 停滞"]
    H --> IC["病态条件<br/>条件数巨大<br/>之字形震荡"]
    H --> FIX["对策"]
...
> 打印提示：完整版此处为流程图；打印版保留为文字复习线索。
> 训练阶段 → 当前 Mini-batch → 实时计算 μ, σ → 归一化 + γ, β → 更新 γ, β → 推理阶段 → 单样本输入 → 使用 Running μ, σ
