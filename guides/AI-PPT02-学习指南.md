...
- **is-a**：子类/实例关系，构成**分类层级**（Taxonomic Hierarchy）。
...
- **继承 vs 默认**：继承是严格层级传递；默认值可被具体, 具体事实覆盖，推理**非单调**。
...
- **vs 语义网络**：SN 用标记弧，**三元及以上关系**需 Reification 拆成多条二元弧；CG 一个关系节点可连多个概念，保持原子性。
