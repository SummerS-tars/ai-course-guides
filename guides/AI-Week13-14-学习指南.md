...
| **★必考 1** | **确定性因子 (CF) 计算题**——公式 + 多规则合成手算 |
| **★必考 2** | **消解证明大题**——NL → FOL → CNF → Resolution → 空子句 |
| **明确不考** | **深度生成模型**（VAE/扩散/流匹配等，Week 12 已标注） |
| **概念级** | PageRank、模糊逻辑、贝叶斯框架、Prolog 机制 |
...
| 术语 | 大白话解释 | 生活类比 |
|------|-----------|----------|
| 🔗 **确定性因子 (CF)** | 用 $[-1,1]$ 数值表示对命题的信任程度 | 医生说「八成像是感冒」——不是严格概率 |
| 🔗 **$CF(E,e)$** | 观测事实 $e$ 下，证据 $E$ 有多可信 | 体温计读数让你多相信「发烧」 |
| 🔗 **$CF(H,E)$** | 若 $E$ 肯定成立，规则推 $H$ 的强度 | 专家写的规则置信度 0.7 |
| 🔗 **$CF(H,e)$** | 单条规则推出的结论强度 | $CF(E,e) \times CF(H,E)$ |
| 🔗 **$CF_{comb}$** | 多条规则指向同一 $H$ 时的合成 | 两个医生意见怎么合并 |
| 🔗 **贝叶斯后验** | 先验 × 似然 → 证据更新后的信念 | 收到新化验单后更新诊断 |
| 🔗 **MAP / L2 正则** | 最大后验 ≈ 损失 + 权重平方惩罚 | 「偏好简单模型」的数学版 |
| 🔗 **马尔可夫性** | 下一状态只依赖当前，与更早历史无关 | 下棋：只关心当前局面 |
| 🔗 **PageRank** | 网页图上随机游走的稳态分布 = 质量分 | 被名站链接的网页更「重要」 |
| 🔗 **隶属度 $\mu(x)$** | 元素属于模糊集合的程度 $[0,1]$ | 「比较高」不是非 0 即 1 |
| 🔗 **命题逻辑** | 原子命题 + 连接词，无个体/量词 | 只能写 $P$、$Q$，不能说「所有人」 |
| 🔗 **一阶谓词逻辑 (FOL)** | 常元/变元/谓词/量词，可表达个体关系 | 「所有人都会死，苏格拉底是人」 |
| 🔗 **霍恩子句** | 至多一个正文字的子句；Prolog 的基础 | `A :- B, C.` |
| 🔗 **CNF / 子句集** | 合取范式：子句的合取，每子句是文字的析取 | 消解算法的输入格式 |
| 🔗 **Skolem 化** | 用 Skolem 常元/函数消去 $\exists$ | 「存在某人」→ 给个具体代表 |
| 🔗 **合一 (Unification)** | 找替换使两表达式相同 | 把规则里的 $x$ 换成 `socrates` |
| 🔗 **消解 (Resolution)** | 互补文字消去，产生新子句 | 反证法：假设否定，推出矛盾 |
| 🔗 **空子句 $\square$** | 消解成功标志：矛盾 | 证明原命题必真 |
| 🔗 **CWA / OWA** | 封闭世界：未知=假；开放世界：未知=未知 | Prolog 否定即失败 vs 知识图谱 |
...
Week 13–14 是 **符号主义推理核心**，承接 Week 12 知识表示铺垫，通向 Week 15 CLIPS：
...
| 转折 | Week 13 | Week 14 |
|------|---------|---------|
| 不确定性 → 确定性逻辑 | CF 量化规则强度 | 命题/谓词逻辑形式化 |
| 启发式 → 严格证明 | MYCIN 风格代数 | 消解反证法 |
| 铺垫 → 工具 | 产生式规则的不确定性 | Prolog 霍恩子句 + 反向推理 |
...
**核心流程**：先验 $P(H)$ → 观测似然 $P(E|H)$ → 后验 $P(H|E) = \frac{P(E|H)P(H)}{P(E)}$ → 结合损失函数做决策（期望效用最大 / 期望损失最小）。

> **追问：CF 和贝叶斯都是「不确定」，为何课程两套都讲？**
>
> **贝叶斯**遵循概率公理，$P(H)+P(\neg H)=1$，适合有统计数据的场景；**CF** 是 MYCIN 的启发式代数，区分「信任增长 MB」与「不信任增长 MD」，允许 $CF=0$（既不信也不反信），更贴近专家直觉但**非严格概率**。期末 **必考 CF 手算**，贝叶斯侧重概念（含 L2 联系）。

**L2 正则 = 高斯先验**：

$$\log P(w|Data) = \log P(Data|w) + \log P(w)$$
...
1. 构造 $S = KB \cup \{\neg E\}$
2. 将 $S$ 中所有公式化为 **CNF 子句集**
3. 反复应用消解规则
4. 若推出 **空子句 $\square$** → $S$ 不可满足 → $\neg E$ 假 → **$E$ 必真**
...
| 4 | 量词左移（前束化） | 所有量词移到最左 |
| 5 | Skolem 化消 $\exists$ | 外层无 $\forall$：用常元；在 $\forall$ 辖域内：用 Skolem 函数 |
| 6 | 略去全称量词 | 默认所有变元全称量化 |
