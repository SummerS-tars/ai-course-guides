---
name: ai-course-notebooklm
description: >-
  人工智能(H)课程按周学习资料流水线：设计 NotebookLM 单问 manifest、运行 nlm-collect
  采集落盘、通读 raw 产出知识图谱、叙事化整合学习指南。在用户提到 AI课程、Week学习指南、
...
Phase 2   按图谱写 guides/AI-Week*-学习指南.md
...
1. 对齐本地 `1_人工智能H/` 与 NotebookLM source list
2. 更新 `guides/AI课程-14周内容梳理.md` 进度
3. 标注缺失周次
...
- 复杂主题拆开（如 BP 回归 / Softmax+CE / 隐层 δ 各一问）
- 字段：`id`, `layer`, `priority`, `title`, `prompt`, `clear_conversation`
- 模板：`templates/manifest-template.json`
- 范例：`notebooklm-raw/manifests/week3-4.json`

### Prompt 必含

中文；点名周次+课件编号；要求标注来源；L1 要类比、L3 要数值例；禁止一 prompt 多问。
...
范例：`notebooklm-raw/week3-4/knowledge-graph.md`
...
- 每个大模块（BP、CNN…）先有**全景节**（学什么、学完能做什么），再进公式
- 章级「叙事线」+ 节级「本节要回答」+ 小结→承接
- ≥3 张 mermaid、≥2 组易混对比表、≥3 处追问/直观理解块

**产出**：`guides/AI-Week{N}-学习指南.md` 或 `guides/AI-Week{N-M}-学习指南.md`

**标杆**：`guides/AI-Week1-2-学习指南.md`（定稿）、`guides/AI-Week3-4-学习指南.md`（迭代中）
...
跑 `docs/checklist.md`，更新 `guides/AI课程-14周内容梳理.md`，用户确认。
...
```
.cursor/skills/ai-course-notebooklm/
├── SKILL.md
